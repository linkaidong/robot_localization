...
Scan2MapLocation::Scan2MapLocation() : private_nh("~"), tf_listener_(tfBuffer_) {
    ...
    map_pointcloud_publisher_ = location_nh.advertise<sensor_msgs::PointCloud2>("map_pointcloud", 10);

    scan_pointcloud_publisher_ = location_nh.advertise<sensor_msgs::PointCloud2>("scan_pointcloud", 10);

    removal_pointcloud_publisher_ = location_nh.advertise<sensor_msgs::PointCloud2>("removal_pointcloud", 10);

    map_scan_publisher_ = location_nh.advertise<sensor_msgs::LaserScan>("map_scan", 10);

    // 注意，这里的发布器，发布的数据类型为 PointCloudT
    // ros中自动做了 PointCloudT 到 sensor_msgs/PointCloud2 的数据类型的转换
    icp_pointcloud_publisher_ = location_nh.advertise<PointCloudT>("icp_pointcloud", 1, this);

    rotate_pointcloud_publisher_ = location_nh.advertise<PointCloudT>("rotate_pointcloud", 1, this);

    location_publisher_ = location_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("location_match", 1, this);

    relocate_tranform_visuial_publisher_ = location_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("relocate_visuial_pose", 1, this);

    rotate_robotpose_publisher_ = location_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("rotate_robot_pose", 1, this);

    relocate_initialpose_publisher_ = location_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1, this);

    location_info_publisher_ = location_nh.advertise<ucar_msgs::LocationInfo>("location_info", 1, this);

//    relocalization_srv_ = location_nh.advertiseService("relocalization", &Scan2MapLocation::RelocalizeCallback, this);
...
void Scan2MapLocation::InitParams() {
    ...
    private_nh.param<bool>("save_pcd", save_pcd_, true);
    ...
    // 定位禁区、
    private_nh.param<std::vector<double>>("location_restricted_zone", location_restricted_zone_, {});
    ...
    private_nh.param<int>("Loss_Num_Threshold", Loss_Num_Threshold_, -1);   //丢失阈值，丢失次数大于此值进入重定位
}
 ...
 * 重定位服务回调函数
*/
bool Scan2MapLocation::RelocalizeCallback(ucar_msgs::Relocate::Request& req, ucar_msgs::Relocate::Response& res)
{
    ros::Rate rate(10);
...
void Scan2MapLocation::laserCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
    ...
    start_time_ = std::chrono::steady_clock::now(); //计算获取变换时间
...
//        if (!GetTransform(map_to_base_, map_frame_, base_frame_, ros::Time::now())) {
            ROS_WARN("Did not get base pose at now");
            return;
        }
...
//        map_to_lidar_ =  map_to_base_;//获得map到lidar变换，用于雷达数据转地图数据
    ...
    if (if_debug_){ std::cout << "获取map到base和lidar坐标变换用时: " << time_used_.count() << " 秒。" << std::endl; }
...
bool Scan2MapLocation::ScanMatchWithICP(Eigen::Isometry3d &trans , PointCloudT::Ptr &cloud_scan_msg, PointCloudT::Ptr &cloud_map_msg)
{
    ...
    ucar_msgs::LocationInfo location_info_msg;
